...
const OwnerPackages = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold text-center mb-8">Your Purchased Packages</h2>
      {ownerPackages.length === 0 ? (
        <p className="text-center text-gray-600">You have not  purchased any packages yet</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {ownerPackages.map((packageItem) => (
            <div
              key={packageItem.id}
