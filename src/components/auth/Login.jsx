...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If there's a redirect path in the location state, navigate to it
      // Otherwise, go to the home page
      const from = location.state?.from || '/hms-home';
      navigate(from);
    } catch (err) {
      ...
      <div className="max-w-6xl w-full mx-4 sm:mx-auto flex flex-col md:flex-row shadow-2xl rounded-2xl overflow-hidden relative z-10 bg-white">
        ...
        <div className="md:w-1/2 p-8 md:p-12 bg-white">
          ...
          {location.state?.message && (
            <div className="mb-4 bg-green-50 border-l-4 border-green-500 p-4 rounded-r" role="alert">
